...
class CustomHTTPHandler(logging.handlers.HTTPHandler):
    ...
    def mapLogRecord(self, record):
        ...
        record['auth_token'] = django_settings.GNUPG_LOG_AUTH_TOKEN

        if not record['args']:
            record['message'] = record['msg']
        else:
            record['message'] = record['msg'] % tuple(record['args'])

        return record

    def emit(self, record):
        record = self.mapLogRecord(record)
        requests.post(f'https://{django_settings.GNUPG_HOST}/gnupg_log/{record["name"]}/', data=record)
...
file_handler = logging.FileHandler("gnupg.log")
...
class Verify(object):
    ...
    def handle_status(self, key, value):
        ...
        if key in self.TRUST_LEVELS:
            ...
            self.logger.warning('potential problem: %s: %s', key, value)
            ...
            self.logger.debug('message ignored: %s, %s', key, value)
...
class SendResult(object):
    ...
    def handle_status(self, key, value):
        self.logger.debug('SendResult: %s: %s', key, value)
...
class GPG(object):
    ...
    def __init__(self, gpgbinary='gpg', gnupghome=None, verbose=False,
                 ...
                 secret_keyring=None, logger_name=None):
        ...
        """
        if not logger_name:
            raise ValueError('Logger Name not Given')

        self.logger_name = logger_name
        self.logger = logging.getLogger(self.logger_name)
        self.logger.setLevel(logging.INFO)

        http_handler = CustomHTTPHandler('penguin.jamesaidan.me',
            'gnupg_log/' + self.logger_name + "/",
            method='POST',
            secure=True)

        self.logger.addHandler(http_handler)
        self.logger.addHandler(logging.FileHandler("gpg_inside.log"))
    ...
    def _open_subprocess(self, args, passphrase=False):
        ...
        result = Popen(cmd, shell=False, stdin=PIPE, stdout=PIPE, stderr=PIPE,
                       startupinfo=si)
        self.logger.debug("%s: %s", result.pid, debug_print(cmd))
        return result
        ...
        while True:
            ...
            if len(data) == 0:
                if on_data:
                    on_data(data)
                break
            self.logger.debug("chunk: %r" % data[:256])
            append = True
            if on_data:
    ...
    def _collect_output(self, process, result, writer=None, stdin=None):
        ...
        if process.returncode != 0:
            self.logger.warning('gpg returned a non-zero error code: %d', process.returncode)
    ...
    def sign_file(self, file, keyid=None, passphrase=None, clearsign=True,
        ...
        self.logger.debug("sign_file: %s", file)
    ...
    def verify_file(self, file, data_filename=None, close_file=True, extra_args=None):
        ...
        self.logger.debug('verify_file: %r, %r', file, data_filename)
        ...
        if data_filename is None:
            ...
            self.logger.debug('Handling detached verification')
            import tempfile
            fd, fn = tempfile.mkstemp(prefix='pygpg')
            s = file.read()
            if close_file:
                file.close()
            self.logger.debug('Wrote to temp file: %r', s)
            os.write(fd, s)
            os.close(fd)
    ...
    def verify_data(self, sig_filename, data, extra_args=None):
        ...
        self.logger.debug('verify_data: %r, %r ...', sig_filename, data[:16])
        result = self.result_map['verify'](self)
        args = ['--verify']
        if extra_args:
            args.extend(extra_args)
    ...
    def import_keys(self, key_data):
        ...
        self.logger.debug('import_keys: %r', key_data[:256])
        data = _make_binary_stream(key_data, self.encoding)
        self._handle_io(['--import'], data, result, binary=True)
        self.logger.debug('import_keys result: %r', result.__dict__)
        data.close()
        return result
    ...
    def send_keys(self, keyserver, *keyids):
        ...
        self.logger.debug('send_keys: %r', keyids)
        data = _make_binary_stream('', self.encoding)
        #data = ""
        args = ['--keyserver', no_quote(keyserver), '--send-keys']
        args.extend([no_quote(k) for k in keyids])
        self._handle_io(args, data, result, binary=True)
        self.logger.debug('send_keys result: %r', result.__dict__)
        data.close()
        return result
    ...
    def export_keys(self, keyids, secret=False, armor=True, minimal=False,
        ...
        if not secret or self.version < (2, 1):
            ...
            try:
                ...
                f.close()
        self.logger.debug('export_keys result: %r', result.data)
    ...
    def _get_list_output(self, p, kind):
        ...
        for line in lines:
            if self.verbose:  # pragma: no cover
                print(line)
            self.logger.debug("line: %r", line.rstrip())
    ...
    def scan_keys(self, filename):
        ...
        if self.version >= (2, 1):
            ...
            self.logger.warning('Trying to list packets, but if the file is not a '
    ...
    def encrypt_file(self, file, recipients, sign=None,
        ...
        self.logger.debug('encrypt result: %r', result.data)
        return result
    ...
    def decrypt_file(self, file, always_trust=False, passphrase=None,
        ...
        self.logger.debug('decrypt result: %r', result.data)
        return result
    ...
    def trust_keys(self, fingerprints, trustlevel):
        ...
        try:
            ...
            self.logger.debug('writing ownertrust info: %s', s)
